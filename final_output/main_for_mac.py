...
def API():
    ...
    ok_button = tk.Button(key_window, text='finish', command=partial(callbutton, key_window),
    ...
    more.bind('<Enter>', lambda e:more.config(cursor=''))
    ...
    def show_information(event):
        ...
        for coor in cooridnates:
            if coor[0] <= cx <= coor[2] and coor[1] <= cy <=coor[3]:
                ...
                ref.tag_bind('start', '<Enter>', lambda e:ref.config(cursor=''))
                ref.tag_bind('start', '<Leave>', lambda e: ref.config(cursor=''))
                ref.tag_bind('end', '<Enter>', lambda e:ref.config(cursor=''))
                ref.tag_bind('end', '<Leave>', lambda e: ref.config(cursor=''))
                ref.tag_bind('info', '<Enter>', lambda e:ref.config(cursor=''))
